var router = {
    initRouter: function () {
         ...
         */
        Path.map('#/login').to(loginPage.onPageLoad.bind(loginPage)).enter(this.preparePage);
        Path.map('#/events').to(eventsPage.onPageLoad).enter(this.preparePage);
        Path.map('#/presentations').to(presentationPage.onPageLoad).enter(this.preparePage);
        Path.map('#/chat').to(chatPage.onPageLoad.bind(chatPage)).enter(this.preparePage);
